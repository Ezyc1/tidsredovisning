    ...
    try {
       ...
       $db->beginTransaction(); 
        ...
        if($svar->getStatus()===200){
            ...
            $retur.= "<p class='error'>Hämta en aktivitet misslyckades, status "
                    . $svar->getStatus(). "returnerades istället för förväntat 200</p>";
        }

        //Missyckas med att hämta pot med if +1
        $nyttId++;
        $svar= hamtaEnskildAktivitet("$nyttId");
        if($svar->getStatus()===400){
            $retur.= "<p class='ok'>Hämta en aktivitet med id som saknas misslyckades, som förväntat</p>";
        }else{
            $retur.= "<p class='error'>Hämta en aktivitet med id som saknas misslyckades, status "
...
if($svar->getStatus()===400){
    ...
    $svar=uppdateraAktivitet("3", " ");
     if($svar->getStatus()===400){
         $retur.= "<p class='ok'>Uppdatera tom aktivitet misslyckades, som förväntat</p>";
     }else{
         $retur.= "<p class='error'>Uppdatera tom aktivitet misslyckades, status ". $svar->getStatus()
                 . "istället för förväntat 400</p>";
     }

    //uppdatara med samma information misslyckas
    ...
    //Misslyckas med att uppdatera till en aktivitets som redan finns
    $svar=sparaNyAktivitet($aktivitet);
     if($svar->getStatus()===200){
        $nyttId=$svar->getContent()->id;
     }else{
     ...
     $svar=uppdateraAktivitet("$nyttId", "NY ". $aktivitet);
  if($svar->getStatus()===400){
     $retur.= "<p class='ok'>Uppdatera aktivitet till en redan befintligt misslyckades</p>";
 }else{
     $retur.= "<p class='error'>Uppdatera aktivitet till en redan befintligt misslyckades<br>"
     . "status:" . $svar->getStatus() . " returnerades med följande innehåll:<br> "
     . print_r($svar->getContent(), true) . "</p>";
 }
